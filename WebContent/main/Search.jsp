...
function Search(accountId){
	...
	var token="RGAPI-b040a33f-c873-4de5-87d3-c201d2adc753";
	...
	$.ajax({
			...
			$.each(Matches_sort, function(i,ee) {
		 			 ...
		 			 $.ajax({
			 			...
			 			type:"GET",
			 		 	dataType:"json",
			 		 	data:{"URL":riot+"/lol/match/v4/matches/"+Matches_sort[i]},
			 		 	headers:{"X-Riot-Token":token},
			 		 	success:function(json){
			 		 			for(var e=0;e<10;e++){
 		    	...
 		    	if(json[0]){
					if(json[0].queueType=="RANKED_SOLO_5x5"){
						$('#mainBox .tier_imgbox').append("<img class='image' src='https://raw.githubusercontent.com/ByeonJaeJeong/LOLSearch/master/WebContent/img/"+json[0].tier+".png'>");
		 		 		$('#mainBox .tier_Rank').append("<span>"+json[0].tier+" "+json[0].rank+"</span>");
		 		 		$('#mainBox .tier_info').append("<b>"+json[0].leaguePoints+"LP</b><span>/"+json[0].wins+"승"+json[0].losses+"패</span><br>"
		 		 				+"<span>승률"+Math.floor(json[0].wins/(json[0].wins+json[0].losses)*100)+"%</span><br>");
		 		 		if(json[0].miniSeries){
		 		 				var winlose=new Array();
		 		 					winlose=(json[0].miniSeries.progress).split("");
		 		 					console.log(winlose[0]);
		 		 			$('#mainBox .tier_info').append("<span>승급전 진행중</span><br>");
		 		 			for(var dat=0;dat<winlose.length;dat++)
		 		 			$('#mainBox .tier_info').append("<div class='"+winlose[dat]+"'>"+winlose[dat]+"</div>");
		 		 			}
		 		 			$('#mainBox>.legueName').append("<span>직스의 칼날단</span>");
					}//json[0]이 솔랭일때
					else{
						$('#subBox .tier_imgbox').append("<img class='image' src='https://raw.githubusercontent.com/ByeonJaeJeong/LOLSearch/master/WebContent/img/"+json[0].tier+".png'>");
		 		 		$('#subBox .tier_Rank').append("<span>"+tier+" "+rank+"</span>");
		 		 		$('#subBox .tier_info').append("<b>"+json[0].leaguePoints+"LP</b><span>/"+json[0].wins+"승"+json[0].losses+"패</span><br>"
		 		 				+"<span>승률"+Math.floor(json[0].wins/(json[0].wins+json[0].losses)*100)+"%</span><br>");
		 		 		if(json[0].miniSeries){
		 		 				var winlose=new Array();
		 		 					winlose=(json[0].miniSeries.progress).split("");
		 		 					console.log(winlose[0]);
		 		 			$('#subBox .tier_info').append("<span>승급전 진행중</span><br>");
		 		 			for(var dat=0;dat<winlose.length;dat++)
		 		 			$('#subBox .tier_info').append("<div class='"+winlose[dat]+"'>"+winlose[dat]+"</div>");
		 		 			}
		 		    	setTimeout(5);
		 		 			$('#subBox>.legueName').append("<span>직스의 칼날단</span>");
					}//json[0]이 자랭일때
 		    	  }//json[0]에 데이터가 있을때
 		  	 if(json[1]){
 			  if(json[1].queueType=="RANKED_SOLO_5x5"){
 				 $('#mainBox .tier_imgbox').append("<img class='image' src='https://raw.githubusercontent.com/ByeonJaeJeong/LOLSearch/master/WebContent/img/"+json[1].tier+".png'>");
	 		 		$('#mainBox .tier_Rank').append("<span>"+json[1].tier+" "+json[1].rank+"</span>");
	 		 		$('#mainBox .tier_info').append("<b>"+json[1].leaguePoints+"LP</b><span>/"+json[1].wins+"승"+json[1].losses+"패</span><br>"
	 		 				+"<span>승률"+Math.floor(json[1].wins/(json[1].wins+json[1].losses)*100)+"%</span><br>");
	 		 		if(json[1].miniSeries){
	 		 				var winlose=new Array();
	 		 					winlose=(json[1].miniSeries.progress).split("");
	 		 					console.log(winlose[1]);
	 		 			$('#mainBox .tier_info').append("<span>승급전 진행중</span><br>");
	 		 			for(var dat=0;dat<winlose.length;dat++)
	 		 			$('#mainBox .tier_info').append("<div class='"+winlose[dat]+"'>"+winlose[dat]+"</div>");
	 		 			}
 				 ...
 				 $('#subBox .tier_imgbox').append("<img class='image' src='https://raw.githubusercontent.com/ByeonJaeJeong/LOLSearch/master/WebContent/img/"+json[1].tier+".png'>");
	 		 		$('#subBox .tier_Rank').append("<span>"+tier+" "+rank+"</span>");
	 		 		$('#subBox .tier_info').append("<b>"+json[1].leaguePoints+"LP</b><span>/"+json[1].wins+"승"+json[1].losses+"패</span><br>"
	 		 				+"<span>승률"+Math.floor(json[1].wins/(json[1].wins+json[1].losses)*100)+"%</span><br>");
	 		 		if(json[0].miniSeries){
	 		 				var winlose=new Array();
	 		 					winlose=(json[1].miniSeries.progress).split("");
...
<div class="Search">
<input type="button" value="전적검색" onclick="Search(accountId)">
</div>
</div>
</div>
<!--  header -->
