...
public class configAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			MultipartRequest mutip = new MultipartRequest(request,
					request.getSession().getServletContext().getRealPath("/img/User-profileicon"), maxSize, "UTF-8",
					new DefaultFileRenamePolicy());

			user_id = mutip.getParameter("user_id");
			icon=mutip.getParameter("in-icon");
			if (icon.equals("off")) {
				...
				if (fileName != null) {
					...
					udao.updateprofile(user_id, path + "no-img.png");
					session.setAttribute("userprofileicon", path + "no-img.png");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
